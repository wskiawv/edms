app.controller('UserAddEditCtrl', ['$scope','$http', '$modalInstance','toaster','model',function($scope,$http,$modalInstance,toaster,model) {
	$scope.toaster = {
	        type: 'success',
	        title: '温馨提示',
	        text: '保存成功'
...
app.controller('UserCtrl', ['$scope','$http','$modal','toaster','ngDialog',function($scope,$http,$modal,toaster,ngDialog) {
	//选中记录数组
	$scope.mySelections = [];
	$scope.filterOptions = {
	        filterText: "",
	...
	$scope.add=function(){
        ...
        var modalInstance = $modal.open({
            templateUrl: 'tpl/sys/userModal.html',
	...
	$scope.remove=function(){
		var url=appPath+"/sys/Users/delete";
		var records=$scope.mySelections;
		var len=records!=null?records.length:0;
		if(len==0){
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            var data;
            var url=appPath+"/sys/Users/showList?pageSize="+pageSize+"&currentPage="+page;
            if (searchText) {                
                $http.get(url).success(function (response) {		
	...
	$scope.grid = {
        ...
        columnDefs: [{field:'id', displayName:'序号'},
                     ...
                     {field:'loginName', displayName:'登陆名'},
                     {field:'password', displayName:'登陆密码'},
                     ...
                     {field:'rmb', displayName:'人民币账户'},
                     {field:'usd', displayName:'美元账户'},
                     {field:'loginDate', displayName:'最近登录时间'},
                     {field:'uuid', displayName:'激活编号'},
                     {field:'activateCode', displayName:'激活码'},
