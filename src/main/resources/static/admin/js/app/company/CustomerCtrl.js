app.controller('CustomerAddEditCtrl', ['$scope','$http', '$modalInstance','toaster','model',function($scope,$http,$modalInstance,toaster,model) {
	$scope.toaster = {
	        type: 'success',
	        title: '温馨提示',
	        text: '保存成功'
	};
	$scope.customer=model;
    $scope.pop = function(){
        toaster.pop($scope.toaster.type, $scope.toaster.title, $scope.toaster.text);
    };
	//保存
    $scope.save = function () {
    	var url=appPath+"/company/Customers/save";    	
    	var params=$scope.customer;
    	var id=params.id!=null&&params.id!=undefined&&params.id!=''?params.id:null;
        if(id!=null){
            url=appPath+"/company/Customers/update";
        }
    	debugger;
...
app.controller('CustomerCtrl', ['$scope','$http','$modal','toaster','ngDialog',function($scope,$http,$modal,toaster,ngDialog) {
	//选中记录数组
	$scope.mySelections = [];
	$scope.filterOptions = {
	        filterText: "",
	...
	$scope.add=function(){
        ...
        var modalInstance = $modal.open({
            templateUrl: 'tpl/company/customerModal.html',
	...
	$scope.remove=function(){
		var url=appPath+"/company/Customers/delete";		
		var records=$scope.mySelections;
		var len=records!=null?records.length:0;
		if(len==0){
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            var data;
            var url=appPath+"/company/Customers/showList?pageSize="+pageSize+"&currentPage="+page;
            if (searchText) {                
                $http.get(url).success(function (response) {		
