app.controller('EmployeeAddEditCtrl', ['$scope','$http', '$modalInstance','toaster','model',function($scope,$http,$modalInstance,toaster,model) {
	$scope.toaster = {
	        type: 'success',
	        title: '温馨提示',
	        text: '保存成功'
...
app.controller('EmployeeCtrl', ['$scope','$http','$modal','toaster','ngDialog',function($scope,$http,$modal,toaster,ngDialog) {
	//选中记录数组
	$scope.mySelections = [];
	$scope.filterOptions = {
	        filterText: "",
	...
	$scope.add=function(){
        ...
        var modalInstance = $modal.open({
            templateUrl: 'tpl/company/employeeModal.html',
	...
	$scope.remove=function(){
		var url=appPath+"/company/Employees/delete";
		var records=$scope.mySelections;
		var len=records!=null?records.length:0;
		if(len==0){
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            var data;
            var url=appPath+"/company/Employees/showList?pageSize="+pageSize+"&currentPage="+page;
            if (searchText) {
                $http.get(url).success(function (response) {
	...
	$scope.grid = {
        ...
        columnDefs: [{field:'id', displayName:'序号'},
                     {field:'employeeName', displayName:'姓名'},
                     ...
                     {field:'register_date', displayName:'注册时间'},
