app.controller('NewsAddEditCtrl', ['$scope','$http', '$modalInstance','toaster','model',function($scope,$http,$modalInstance,toaster,model) {
	$scope.toaster = {
	        type: 'success',
	        title: '温馨提示',
	        text: '保存成功'
	...
	//保存
    $scope.save = function () {
    	var url=appPath+"/company/Newses/save";    	
    	var params=$scope.news;
    	var id=params.id!=null&&params.id!=undefined&&params.id!=''?params.id:null;
    	if(id!=null){
    	    url=appPath+"/company/Newses/update";
    	}
    	debugger;
...
app.controller('NewsCtrl', ['$scope','$http','$modal','toaster','ngDialog',function($scope,$http,$modal,toaster,ngDialog) {
	//选中记录数组
	$scope.mySelections = [];
	$scope.filterOptions = {
	        filterText: "",
	...
	$scope.add=function(){
        ...
        var modalInstance = $modal.open({
            templateUrl: 'tpl/company/newsModal.html',
            controller: 'NewsAddEditCtrl',
            size: 'lg',
            backdrop:'static',
            resolve: {
            	model: function () {
	...
	$scope.remove=function(){
		var url=appPath+"/company/Newses/delete";		
		var records=$scope.mySelections;
		var len=records!=null?records.length:0;
		if(len==0){
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            var data;
            var url=appPath+"/company/Newses/showList?pageSize="+pageSize+"&currentPage="+page;
            if (searchText) {                
                $http.get(url).success(function (response) {		
	...
	$scope.grid = {
        ...
        columnDefs: [{field:'id', displayName:'序号'},
                     ...
                     {field:'image', displayName:'图片'},
                     {field:'video', displayName:'视频'},
                     ...
                     {field:'approve_by', displayName:'审批人'},
                     {field:'approve_date', displayName:'审批时间'},
