app.controller('SolutionAddEditCtrl', ['$scope','$http', '$modalInstance','toaster','model',function($scope,$http,$modalInstance,toaster,model) {
	$scope.toaster = {
	        type: 'success',
	        title: '温馨提示',
	        text: '保存成功'
	...
	//保存
    $scope.save = function () {
    	var url=appPath+"/company/Solutions/save";
    	var params=$scope.solution;
    	var id=params.id!=null&&params.id!=undefined&&params.id!=''?params.id:null;
        if(id!=null){
            url=appPath+"/company/Solutions/update";
        }
    	debugger;
...
app.controller('SolutionCtrl', ['$scope','$http','$modal','toaster','ngDialog',function($scope,$http,$modal,toaster,ngDialog) {
	//选中记录数组
	$scope.mySelections = [];
	$scope.filterOptions = {
	        filterText: "",
	...
	$scope.add=function(){
        ...
        var modalInstance = $modal.open({
            templateUrl: 'tpl/company/solutionModal.html',
            controller: 'SolutionAddEditCtrl',
            size: 'lg',
            backdrop:'static',
            resolve: {
            	model: function () {
	...
	$scope.remove=function(){
		var url=appPath+"/company/Solutions/delete";
		var records=$scope.mySelections;
		var len=records!=null?records.length:0;
		if(len==0){
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            var data;
            var url=appPath+"/company/Solutions/showList?pageSize="+pageSize+"&currentPage="+page;
            if (searchText) {                
                $http.get(url).success(function (response) {		
	...
	$scope.grid = {
        ...
        columnDefs: [{field:'id', displayName:'序号'},
                     {field:'name', displayName:'方案名称'},
                     {field:'introduce', displayName:'方案介绍'},
                     {field:'remarks', displayName:'备注'},
                     {field:'pdf', displayName:'方案介绍Pdf'},
                     {field:'video', displayName:'方案介绍视频'},
