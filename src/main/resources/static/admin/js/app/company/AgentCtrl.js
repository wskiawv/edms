app.controller('AgentAddEditCtrl', ['$scope','$http', '$modalInstance','toaster','model',function($scope,$http,$modalInstance,toaster,model) {
	$scope.toaster = {
	        type: 'success',
	        title: '温馨提示',
	        text: '保存成功'
	...
	//保存
    $scope.save = function () {
    	var url=appPath+"/company/Agents/save";
    	var params=$scope.agent;
    	var id=params.id!=null&&params.id!=undefined&&params.id!=''?params.id:null;
        if(id!=null){
            url=appPath+"/company/Agents/update";
        }
    	debugger;
...
app.controller('AgentCtrl', ['$scope','$http','$modal','toaster','ngDialog',function($scope,$http,$modal,toaster,ngDialog) {
	//选中记录数组
	$scope.mySelections = [];
	$scope.filterOptions = {
	        filterText: "",
	...
	$scope.add=function(){
        ...
        var modalInstance = $modal.open({
            templateUrl: 'tpl/company/agentModal.html',
            controller: 'AgentAddEditCtrl',
            size: 'lg',
            backdrop:'static',
            resolve: {
            	model: function () {
	...
	$scope.remove=function(){
		var url=appPath+"/company/Agents/delete";
		var records=$scope.mySelections;
		var len=records!=null?records.length:0;
		if(len==0){
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            var data;
            var url=appPath+"/company/Agents/showList?pageSize="+pageSize+"&currentPage="+page;
            if (searchText) {                
                $http.get(url).success(function (response) {		
	...
	$scope.grid = {
        ...
        columnDefs: [{field:'id', displayName:'序号'},
                     ...
                     {field:'company', displayName:'公司名称'},
                     {field:'address', displayName:'地址'},
                     {field:'website', displayName:'网址'},
                     {field:'phone', displayName:'手机'},
                     {field:'email', displayName:'邮箱'},
                     {field:'fax', displayName:'传真'},
                     {field:'introduce', displayName:'公司简介'},
                     {field:'logo', displayName:'logo'},
                     {field:'tax_number', displayName:'税务登记编号'},
                     {field:'legal_person', displayName:'法人'},
                     {field:'contacts', displayName:'联系人'},
                     {field:'telephone', displayName:'电话号码'},
                     {field:'postal_number', displayName:'邮编'},
